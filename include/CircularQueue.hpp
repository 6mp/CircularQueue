 ...
 public:
  ...
  constexpr CircularQueue(std::initializer_list<Ty> values) {
    static_assert(values.size() <= Size,
                  "initializer_list size must be <= Size");
  }
  ...
  constexpr CircularQueue(It container)
      : m_head(0), m_tail(container.size() - 1), m_storage(Size) {
  ...
  constexpr auto push(Args&&... args) {
    static_assert(std::is_constructible_v<Ty, Args...>,
                  "Ty must be constructable with args...");
